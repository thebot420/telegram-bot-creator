...
class Product(db.Model):
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    name = db.Column(db.String(100), nullable=False)
    description = db.Column(db.String(1024), nullable=True) # NEW: Product description
    ...
    price_tiers = db.relationship('PriceTier', backref='product', lazy=True, cascade="all, delete-orphan") # NEW: Link to price tiers
...
class PriceTier(db.Model):
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    label = db.Column(db.String(100), nullable=False) # e.g., "2 Pack", "500g"
